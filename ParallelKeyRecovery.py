...
# %% [markdown]
# ## Implementation of The (Faulty) AES-128
# %% [markdown]
...
class AES:
    ...
    def change_key(self, master_key):
        ...
        for i in range(4, 4 * 11):
            ...
            if i % 4 == 0:
                byte = self.round_keys[i - 4][0]                             ^ self.sbox[self.round_keys[i - 1][1]]                       ^ self.Rcon[i // 4]
    ...
    def derive_round_keys_from_last_round_key(self, last_rkey):        
        ...
        for i in range(39, -1, -1):
            if i % 4 == 0:
                temp = []
                byte = self.round_keys[i + 4][0]                      ^ self.sbox[self.round_keys[i + 3][1]]                      ^ self.Rcon[i // 4 + 1]
                temp.append(byte)

                for j in range(1, 4):
                    byte = self.round_keys[i + 4][j]                          ^ self.sbox[self.round_keys[i + 3][(j + 1) % 4]]
                    temp.append(byte)
                self.round_keys[i] = temp
            else:
                temp = []
                for j in range(4):
                    byte = self.round_keys[i + 4][j]                          ^ self.round_keys[i + 3][j]
                    temp.append(byte)
                self.round_keys[i] = temp    
